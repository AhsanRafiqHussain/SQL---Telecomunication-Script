CREATE OR REPLACE FUNCTION lte_kpi_abnorm_rel_e_rabs_due_to_ul_resynchronization_failures_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM lte_kpi_abnorm_rel_e_rabs_due_to_ul_resynchronization_failures_sector_hourly
...
ELSE IF count_kpi = 0  THEN
 ...
 --1- Sector 
	...
	insert into lte_kpi_abnorm_rel_e_rabs_due_to_ul_resynchronization_failures_sector_hourly("lte_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","abnorm_rel_e_rabs_due_to_ul_resynchronization_failures")
	Select 
	...
	,lte_pm.minutes
	,_1526729551 
 AS abnorm_rel_e_rabs_due_to_ul_resynchronization_failures
...
message := 'lte KPI abnorm_rel_e_rabs_due_to_ul_resynchronization_failures calculated. '||int_kpi || ' rows are updated for Datetime ' || _date ||'.';
END IF;
END IF;
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION lte_kpi_abnorm_rel_e_rabs_due_to_ul_resynchronization_failures_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
