CREATE OR REPLACE FUNCTION lte_kpi_unavailability_rate_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM lte_kpi_unavailability_rate_sector_hourly
...
ELSE IF count_kpi = 0  THEN
 ...
 --1- Sector 
	...
	insert into lte_kpi_unavailability_rate_sector_hourly("lte_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","unavailability_rate")
	Select 
	...
	,lte_pm.hours
	,lte_pm.minutes
	,roundf((100 * ((l_cell_unavail_dur_sys + l_cell_unavail_dur_manual) / 3600)), 2) AS unavailability_rate 
	FROM lte_temp_information lte_pm
	inner join huawei_pm_4g_algo_cell a ON lte_pm.local_cell_id = a.local_cell_id
...
message := 'lte KPI unavailability_rate calculated. '||int_kpi || ' rows are updated for Datetime ' || _date ||'.';
END IF;
END IF;
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION lte_kpi_unavailability_rate_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
