CREATE OR REPLACE FUNCTION lte_kpi_ul_thp_cqi_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM lte_kpi_ul_thp_cqi_sector_hourly
...
ELSE IF count_kpi = 0  THEN
 ...
 --1- Sector 
	...
	insert into lte_kpi_ul_thp_cqi_sector_hourly("lte_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","ul_thp_cqi")
	Select 
	...
	,lte_pm.minutes
	,CASE 
		WHEN(L_Thrp_Time_Cell_UL_HighPrecision * 0.001) = 0
			THEN 0
		ELSE roundf(((L_Thrp_bits_UL / 1048576) / (L_Thrp_Time_Cell_UL_HighPrecision * 0.001)), 2)
		END AS ul_thp_cqi 
...
message := 'lte KPI ul_thp_cqi calculated. '||int_kpi || ' rows are updated for Datetime ' || _date ||'.';
END IF;
END IF;
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION lte_kpi_ul_thp_cqi_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
