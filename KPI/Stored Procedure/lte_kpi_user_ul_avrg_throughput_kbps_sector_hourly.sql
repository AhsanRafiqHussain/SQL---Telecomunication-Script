CREATE OR REPLACE FUNCTION lte_kpi_user_ul_avrg_throughput_kbps_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM lte_kpi_user_ul_avrg_throughput_kbps_sector_hourly
...
ELSE IF count_kpi = 0  THEN
 ...
 --1- Sector 
	...
	insert into lte_kpi_user_ul_avrg_throughput_kbps_sector_hourly("lte_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","user_ul_avrg_throughput_kbps")
	Select 
	...
	,lte_pm.minutes
	,CASE 
		WHEN l_thrp_time_ue_ul_rmvlasttti = 0
			THEN 0
		ELSE roundf(((l_thrp_bits_ul - l_thrp_bits_ue_ul_lasttti) / l_thrp_time_ue_ul_rmvlasttti), 2)
		END AS user_ul_avrg_throughput_kbps 
...
message := 'lte KPI user_ul_avrg_throughput_kbps calculated. '||int_kpi || ' rows are updated for Datetime ' || _date ||'.';
END IF;
END IF;
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION lte_kpi_user_ul_avrg_throughput_kbps_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
