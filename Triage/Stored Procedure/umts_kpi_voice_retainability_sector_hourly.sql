CREATE OR REPLACE FUNCTION umts_kpi_voice_retainability_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_voice_retainability_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs voice_retainability  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_voice_retainability_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","voice_retainability")
	Select 
	...
	,umts_pm.minutes
	,CASE 
		WHEN (vs_rab_normrel_cs + vs_rab_abnormrel_cs) = 0
			THEN 100
		ELSE 100 - roundf(((vs_rab_abnormrel_cs / (vs_rab_normrel_cs + vs_rab_abnormrel_cs)) * 100), 2)
		END AS voice_retainability
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_voice_retainability_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
