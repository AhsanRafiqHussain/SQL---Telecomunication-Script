CREATE OR REPLACE FUNCTION umts_kpi_rrc_succ_data_reliability_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_rrc_succ_data_reliability_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs rrc_succ_data_reliability  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_rrc_succ_data_reliability_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","rrc_succ_data_reliability")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,(rrc_succconnestab_orgintercall + rrc_succconnestab_orgbkgcall + rrc_succconnestab_tmitrcall + rrc_succconnestab_tmbkgcall + vs_succcellupdt_pagersp + vs_succcellupdt_uldatatrans) - (vs_succcellupdt_orgconvcall_pch + vs_succcellupdt_tmconvcall_pch + vs_succcellupdt_emgcall_pch) + (rrc_succconnestab_orgstrcall + rrc_succconnestab_tmstrcall + rrc_succconnestab_orgsubcall + rrc_succconnestab_unkown + rrc_succconnestab_orghhprsig + rrc_succconnestab_orglwprsig + rrc_succconnestab_callreest + rrc_succconnestab_tmhhprsig + rrc_succconnestab_tmlwprsig) AS rrc_succ_data_reliability
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_rrc_succ_data_reliability_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
