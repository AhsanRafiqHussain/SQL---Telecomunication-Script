CREATE OR REPLACE FUNCTION umts_kpi_unavailtime_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_unavailtime_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs unavailtime  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_unavailtime_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","unavailtime")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,(vs_cell_unavailtime_sys + vs_cell_unavailtime)  AS unavailtime
	FROM umts_temp_information umts_pm
	inner join huawei_pm_3g_algo_cell a ON umts_pm.cell_id = a.cellid
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_unavailtime_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
