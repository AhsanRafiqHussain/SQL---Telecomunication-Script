CREATE OR REPLACE FUNCTION umts_kpi_rrc_cong_rate_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_rrc_cong_rate_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs rrc_cong_rate  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_rrc_cong_rate_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","rrc_cong_rate")
	Select 
	...
	,CASE 
		WHEN vs_rrc_attconnestab_sum = 0
			THEN 0
		ELSE ((vs_rrc_rej_code_cong + vs_rrc_rej_dlce_cong + vs_rrc_rej_ulce_cong + vs_rrc_rej_dlpower_cong + vs_rrc_rej_ulpower_cong + vs_rrc_rej_dliubband_cong + vs_rrc_rej_uliubband_cong) / vs_rrc_attconnestab_sum) * 100
		END AS rrc_cong_rate 
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_rrc_cong_rate_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
