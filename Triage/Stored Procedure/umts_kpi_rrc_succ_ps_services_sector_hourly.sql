CREATE OR REPLACE FUNCTION umts_kpi_rrc_succ_ps_services_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_rrc_succ_ps_services_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs rrc_succ_ps_services  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_rrc_succ_ps_services_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","rrc_succ_ps_services")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,(((rrc_succconnestab_orgbkgcall + rrc_succconnestab_orgintercall + rrc_succconnestab_tmbkgcall + rrc_succconnestab_tmitrcall + vs_succcellupdt_pagersp + vs_succcellupdt_uldatatrans)) - ((vs_succcellupdt_orgconvcall_pch + vs_succcellupdt_emgcall_pch + vs_succcellupdt_tmconvcall_pch)))+((RRC_SuccConnEstab_OrgSubCall + RRC_SuccConnEstab_TmStrCall + RRC_SuccConnEstab_Unkown + RRC_SuccConnEstab_OrgHhPrSig + RRC_SuccConnEstab_OrgLwPrSig + RRC_SuccConnEstab_CallReEst + RRC_SuccConnEstab_TmHhPrSig + RRC_SuccConnEstab_TmLwPrSig)) as  RRC_Succ_PS_Services
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_rrc_succ_ps_services_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
