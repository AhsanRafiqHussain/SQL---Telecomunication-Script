CREATE OR REPLACE FUNCTION umts_kpi_soft_handover_success_rate_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_soft_handover_success_rate_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs soft_handover_success_rate  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_soft_handover_success_rate_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","soft_handover_success_rate")
	Select 
	...
	,umts_pm.minutes
	,CASE 
		WHEN (vs_sho_attrladd + vs_sho_attrldel) = 0
			THEN 100
		ELSE ((vs_sho_succrladd + vs_sho_succrldel) / (vs_sho_attrladd + vs_sho_attrldel)) * 100
		END AS soft_handover_success_rate
	FROM umts_temp_information umts_pm
	inner join huawei_pm_3g_sho_cell a ON umts_pm.cell_id = a.cellid
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_soft_handover_success_rate_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
