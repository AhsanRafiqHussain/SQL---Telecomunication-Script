CREATE OR REPLACE FUNCTION umts_kpi_total_hsdpa_hsupa_meanthroughput_mbytes_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_total_hsdpa_hsupa_meanthroughput_mbytes_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs total_hsdpa_hsupa_meanthroughput_mbytes  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_total_hsdpa_hsupa_meanthroughput_mbytes_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","total_hsdpa_hsupa_meanthroughput_mbytes")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,roundf(((vs_hsdpa_meanchthroughput_totalbytes + vs_hsupa_meanchthroughput_totalbytes) / 1048576), 2) AS total_hsdpa_hsupa_meanthroughput_mbytes
	FROM umts_temp_information umts_pm
	inner join huawei_pm_3g_hsdpa_cell a ON umts_pm.cell_id = a.cellid
	AND umts_pm.rnc = a.netype
	AND a.begintime = _date
	inner join huawei_pm_3g_hsupa_cell b ON umts_pm.cell_id = b.cellid
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_total_hsdpa_hsupa_meanthroughput_mbytes_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
