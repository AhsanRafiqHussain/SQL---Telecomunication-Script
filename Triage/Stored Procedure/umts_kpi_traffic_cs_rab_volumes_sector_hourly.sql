CREATE OR REPLACE FUNCTION umts_kpi_traffic_cs_rab_volumes_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_traffic_cs_rab_volumes_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs traffic_cs_rab_volumes  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_traffic_cs_rab_volumes_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","traffic_cs_rab_volumes")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,(vs_rab_attestab_amr + vs_rab_attestcs_conv_64) AS traffic_cs_rab_volumes
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_traffic_cs_rab_volumes_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
