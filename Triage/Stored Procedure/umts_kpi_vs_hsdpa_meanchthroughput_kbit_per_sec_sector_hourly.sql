CREATE OR REPLACE FUNCTION umts_kpi_vs_hsdpa_meanchthroughput_kbit_per_sec_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_vs_hsdpa_meanchthroughput_kbit_per_sec_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs vs_hsdpa_meanchthroughput_kbit_per_sec  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_vs_hsdpa_meanchthroughput_kbit_per_sec_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","vs_hsdpa_meanchthroughput_kbit_per_sec")
	Select 
	...
	,umts_pm.minutes
	,CASE 
		WHEN cast(vs_hsdpa_meanchthroughput AS VARCHAR) = 'NaN'
			THEN 0
		ELSE vs_hsdpa_meanchthroughput
		END AS vs_hsdpa_meanchthroughput_kbit_per_sec
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_vs_hsdpa_meanchthroughput_kbit_per_sec_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
