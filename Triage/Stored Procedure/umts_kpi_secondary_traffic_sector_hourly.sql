CREATE OR REPLACE FUNCTION umts_kpi_secondary_traffic_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_secondary_traffic_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs secondary_traffic  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_secondary_traffic_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","secondary_traffic")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,(vs_rb_amr_dl_12_2 - vs_amr_erlang_bestcell) / 2 AS secondary_traffic 
	...
	inner join huawei_pm_3g_thruput_cell b ON umts_pm.cell_id = b.cellid
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_secondary_traffic_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
