CREATE OR REPLACE FUNCTION umts_kpi_rnc_cs_rab_rel_trig_by_rnc_aal2_loss_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_rnc_cs_rab_rel_trig_by_rnc_aal2_loss_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs rnc_cs_rab_rel_trig_by_rnc_aal2_loss  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_rnc_cs_rab_rel_trig_by_rnc_aal2_loss_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","rnc_cs_rab_rel_trig_by_rnc_aal2_loss")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,vs_rab_abnormrel_cs_iuaal2 AS rnc_cs_rab_rel_trig_by_rnc_aal2_loss
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_rnc_cs_rab_rel_trig_by_rnc_aal2_loss_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
