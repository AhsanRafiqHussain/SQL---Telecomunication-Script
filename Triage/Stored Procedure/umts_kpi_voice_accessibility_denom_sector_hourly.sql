CREATE OR REPLACE FUNCTION umts_kpi_voice_accessibility_denom_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_voice_accessibility_denom_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs voice_accessibility_denom  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_voice_accessibility_denom_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","voice_accessibility_denom")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,((rrc_attconnestab_emgcall + rrc_attconnestab_orgconvcall + rrc_attconnestab_tmconvcall + vs_attcellupdt_orgconvcall_pch + vs_attcellupdt_emgcall_pch + vs_attcellupdt_tmconvcall_pch) * (vs_rab_attestabcs_conv + vs_rab_attestabcs_str)) AS voice_accessibility_denom 
	--,kpi_rrc_setup_sr_total * rab_att_dq_acc AS voice_accessibility_denom 
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_voice_accessibility_denom_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
