CREATE OR REPLACE FUNCTION umts_kpi_traffic_ps_rab_volumes_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_traffic_ps_rab_volumes_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs traffic_ps_rab_volumes  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_traffic_ps_rab_volumes_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","traffic_ps_rab_volumes")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,(vs_rab_attestabps_int + vs_rab_attestabps_bkg) AS traffic_ps_rab_volumes
	FROM umts_temp_information umts_pm
	inner join huawei_pm_3g_rab_estabps_cell a ON umts_pm.cell_id = a.cellid
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_traffic_ps_rab_volumes_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
