CREATE OR REPLACE FUNCTION umts_kpi_total_call_acc_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_total_call_acc_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs total_call_acc  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_total_call_acc_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","total_call_acc")
	Select 
	...
	,umts_pm.minutes
	,CASE 
		WHEN (rrc_succconnestab_orgconvcall + rrc_succconnestab_tmconvcall + rrc_succconnestab_emgcall + vs_succcellupdt_orgconvcall_pch + vs_succcellupdt_tmconvcall_pch + vs_succcellupdt_emgcall_pch) = 0
			OR (rrc_attconnestab_orgconvcall + rrc_attconnestab_tmconvcall + rrc_attconnestab_emgcall + vs_attcellupdt_orgconvcall_pch + vs_attcellupdt_tmconvcall_pch + vs_attcellupdt_emgcall_pch) = 0
			THEN 0
		ELSE ((vs_rab_attestabcs_conv + vs_rab_attestabcs_str) / ((rrc_succconnestab_orgconvcall + rrc_succconnestab_tmconvcall + rrc_succconnestab_emgcall + vs_succcellupdt_orgconvcall_pch + vs_succcellupdt_tmconvcall_pch + vs_succcellupdt_emgcall_pch) / (rrc_attconnestab_orgconvcall + rrc_attconnestab_tmconvcall + rrc_attconnestab_emgcall + vs_attcellupdt_orgconvcall_pch + vs_attcellupdt_tmconvcall_pch + vs_attcellupdt_emgcall_pch)))
		END AS total_call_acc
	FROM umts_temp_information umts_pm
	inner join huawei_pm_3g_rab_estabcs_cell a ON umts_pm.cell_id = a.cellid
	...
	inner join huawei_pm_3g_rrc_setup_cell c ON umts_pm.cell_id = c.cellid
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_total_call_acc_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
