CREATE OR REPLACE FUNCTION umts_kpi_traffic_r99_mb_ps_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_traffic_r99_mb_ps_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs traffic_r99_mb_ps  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_traffic_r99_mb_ps_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","traffic_r99_mb_ps")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,(vs_rb_ps_bkg_dl_128 + vs_rb_ps_bkg_dl_144 + vs_rb_ps_bkg_dl_16 + vs_rb_ps_bkg_dl_256 + vs_rb_ps_bkg_dl_32 + vs_rb_ps_bkg_dl_384 + vs_rb_ps_bkg_dl_64 + vs_rb_ps_bkg_dl_8 + vs_rb_ps_conv_dl_38_8 + vs_rb_ps_conv_dl_39_2 + vs_rb_ps_conv_dl_40 + vs_rb_ps_conv_dl_42_8 + vs_rb_ps_conv_dl_64 + vs_rb_ps_int_dl_128 + vs_rb_ps_int_dl_144 + vs_rb_ps_int_dl_16 + vs_rb_ps_int_dl_256 + vs_rb_ps_int_dl_32 + vs_rb_ps_int_dl_384 + vs_rb_ps_int_dl_64 + vs_rb_ps_int_dl_8 + vs_rb_ps_str_dl_128 + vs_rb_ps_str_dl_144 + vs_rb_ps_str_dl_16 + vs_rb_ps_str_dl_256_384 + vs_rb_ps_str_dl_32 + vs_rb_ps_str_dl_64 + vs_rb_ps_str_dl_8) / 1.35 AS traffic_r99_mb_ps
	FROM umts_temp_information umts_pm
	inner join huawei_pm_3g_rb_cell a ON umts_pm.cell_id = a.cellid
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_traffic_r99_mb_ps_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
