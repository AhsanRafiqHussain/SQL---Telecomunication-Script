CREATE OR REPLACE FUNCTION umts_kpi_sector_classification_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_sector_classification_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs sector_classification  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_sector_classification_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","sector_classification")
	Select 
	...
	inner join sector_classification_core_border a ON umts_pm.cell = a.cell
	AND umts_pm.nodeb = a.site
	AND a.datetime = _date;
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_sector_classification_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
