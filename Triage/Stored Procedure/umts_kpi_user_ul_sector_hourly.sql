CREATE OR REPLACE FUNCTION umts_kpi_user_ul_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_user_ul_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs user_ul  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_user_ul_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","user_ul")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,(vs_rb_amr_ul_12_2 + vs_rb_amr_ul_10_2 + vs_rb_amr_ul_7_95 + vs_rb_amr_ul_7_4 + vs_rb_amr_ul_6_7 + vs_rb_amr_ul_5_9 + vs_rb_amr_ul_5_15 + vs_rb_amr_ul_4_75 + vs_rb_cs_conv_ul_64) AS user_ul
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_user_ul_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
