CREATE OR REPLACE FUNCTION umts_kpi_total_traffic_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_total_traffic_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs total_traffic  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_total_traffic_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","total_traffic")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,((vs_rb_amr_dl_12_2 + vs_rb_amr_dl_10_2 + vs_rb_amr_dl_7_95 + vs_rb_amr_dl_7_4 + vs_rb_amr_dl_6_7 + vs_rb_amr_dl_5_9 + vs_rb_amr_dl_5_15 + vs_rb_amr_dl_4_75) / 2) + (vs_rb_amrwb_dl_6_60 + vs_rb_amrwb_dl_8_85 + vs_rb_amrwb_dl_12_65 + vs_rb_amrwb_dl_14_25 + vs_rb_amrwb_dl_15_85 + vs_rb_amrwb_dl_18_25 + vs_rb_amrwb_dl_19_85 + vs_rb_amrwb_dl_23_05 + vs_rb_amrwb_dl_23_85) AS total_traffic
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_total_traffic_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
