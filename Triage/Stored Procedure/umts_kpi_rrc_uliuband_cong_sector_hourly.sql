CREATE OR REPLACE FUNCTION umts_kpi_rrc_uliuband_cong_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_rrc_uliuband_cong_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs rrc_uliuband_cong  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_rrc_uliuband_cong_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","rrc_uliuband_cong")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,vs_rrc_rej_uliubband_cong AS rrc_uliuband_cong
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_rrc_uliuband_cong_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
