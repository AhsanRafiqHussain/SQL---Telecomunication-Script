CREATE OR REPLACE FUNCTION umts_kpi_voice_accessibility_num_sector_hourly(_date timestamp without time zone)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	count_kpi := (SELECT count(*) 
		FROM umts_kpi_voice_accessibility_num_sector_hourly
		where begin_time = _date) t;
				

IF count_kpi > 0 THEN 
message := 'UMTS KPIs voice_accessibility_num  Tables contain the latest records for Datetime ' ||_date || '.';	
ELSE IF count_kpi = 0  THEN
	---------------------------------------------------------------------
 --1- Sector 
	---------------------------------------------------------------------
	
	insert into umts_kpi_voice_accessibility_num_sector_hourly("umts_sector_information_id","begin_time","end_time","years","months","weeks","days","hours","minutes","voice_accessibility_num")
	Select 
	...
	,umts_pm.hours
	,umts_pm.minutes
	,((rrc_succconnestab_orgconvcall + rrc_succconnestab_tmconvcall + rrc_succconnestab_emgcall + vs_succcellupdt_orgconvcall_pch + vs_succcellupdt_emgcall_pch + vs_succcellupdt_tmconvcall_pch) * (vs_rab_succestabcs_conv + vs_rab_succestabcs_str)) AS voice_accessibility_num
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION umts_kpi_voice_accessibility_num_sector_hourly(timestamp without time zone)
  OWNER TO postgres;
